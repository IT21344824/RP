...
export default function VerifyPin() {
  const router = useRouter();
  const verifyResetPin = useAuthStore((state) => state.verifyResetPin);
  const isLoading = useAuthStore((state) => state.isLoading);
  const inputRefs = useRef<(TextInput | null)[]>([]);

  // Assume user enters their email from previous screen (or store globally)
  const email = "contact@sheritage.com"; // Replace this with dynamic email input
  ...
  return (
    <SafeAreaView className="flex-1 bg-black px-6 pb-8">
      ...
      <View className="mt-20">
        <Text className="text-white text-center font-bold text-3xl">Check your email</Text>
        <Text className="text-gray-400 text-center mt-2">
          We sent a reset PIN to <Text className="text-gray-200 font-semibold">{email}</Text>
        </Text>
        <Text className="text-gray-400 text-center mt-1">Enter the 5-digit code mentioned in the email</Text>
      </View>
      ...
      <Formik
        ...
        validationSchema={PinSchema}
        onSubmit={(values) => {
          verifyResetPin(email, values.pin, () => router.push("/setPassword" as any));
        }}
      >
        {({ handleChange, handleSubmit, values, errors, touched }) => (
          <>
