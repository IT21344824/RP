...
interface HistoryListProps {
  data: Array<{ id: string; predictedClass: string; imageUrl: string; date: string }>;
}
...
const HistoryList: React.FC<HistoryListProps> = ({ data }) => {
  ...
  return (
    <GestureDetector gesture={panGesture}>
      <Animated.View className="bg-white w-full rounded-t-3xl p-5 shadow-lg" style={animatedStyle}>
        {/* Draggable Indicator */}
        <View className="w-24 h-1 bg-gray-600 rounded-full mx-auto z-10" />
        ...
        {/* Scrollable List */}
        {data.length === 0 ? (
          <Text className="text-center text-gray-500">No predictions available.</Text>
        ) : (
          <FlatList
            data={data}
            keyExtractor={(item) => item.id}
